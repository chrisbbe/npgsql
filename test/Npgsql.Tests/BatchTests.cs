...
public class BatchTests : MultiplexingTestBase
{
    ...
    [Test]
    public async Task Batch_with_error_can_read_all_results()
    {
        await using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, a text UNIQUE");

        await using var batch = new NpgsqlBatch(conn)
        {
            BatchCommands =
            {
                new($"INSERT INTO {table} (a) VALUES (8) RETURNING id"),
                new($"INSERT INTO {table} (a) VALUES (8) RETURNING id"), // Violates unique constraint.
                new($"INSERT INTO {table} (a) VALUES (9) RETURNING id")
            },
        ...
        var ids = new int?[batch.BatchCommands.Count];
        for (var i = 0; i < ids.Length; i++)
        {
            ids[i] = await reader.ReadAsync() ? reader.GetInt32(0) : null;
            try { await reader.NextResultAsync(); } catch {/* Ignore/log */}
        }

        Assert.That(ids, Is.EqualTo(new int?[] {1, null, 3}));
    }
